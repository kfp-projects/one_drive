...
// Custom Modal Dialogs (replaces blocking native alert/confirm to prevent browser blocking and flash bugs)
function showCustomConfirm(title, message, okText = 'Confirmar', cancelText = 'Cancelar') {
    return new Promise((resolve) => {
        ...
        titleEl.innerHTML = `<i class="ph ph-question" style="color: var(--primary-color); font-size: 22px; vertical-align: middle; margin-right: 8px;"></i> ${title}`;
...
function showCustomAlert(title, message, btnText = 'OK') {
    return new Promise((resolve) => {
        const modal = document.getElementById('confirm-modal');
        const titleEl = document.getElementById('confirm-modal-title');
        const msgEl = document.getElementById('confirm-modal-message');
        ...
        titleEl.innerHTML = `<i class="ph ph-info" style="color: var(--primary-color); font-size: 22px; vertical-align: middle; margin-right: 8px;"></i> ${title}`;
        msgEl.innerHTML = message.replace(/\n/g, '<br>');
...
async function applyRenamesFlow() {
    if (!latestReport) {
        await showCustomAlert('Aviso', 'Nenhum relatório carregado. Aguarde o carregamento ou rode um scan.');
        return;
    }
    ...
    if (eligible === 0) {
        await showCustomAlert('Aviso', 'Nenhuma renomeação aplicável encontrada no relatório atual.');
        return;
    }

    const ok = await showCustomConfirm(
        'Confirmar Renomeações',
    ...
    try {
        ...
        await showCustomAlert('Sucesso', msg);
        btn.innerHTML = originalText;
        btn.disabled = false;
    } catch (e) {
        await showCustomAlert('Erro de Conexão', `Erro de conexão: ${e.message}`);
        btn.innerHTML = originalText;
        btn.disabled = false;
    }
}
...
function setupScanner() {
    ...
    btnScan.addEventListener('click', async () => {
        ...
        if (!path) {
            await showCustomAlert('Aviso', 'Por favor, informe o caminho do diretório.');
            return;
        }
        ...
        try {
            ...
            if (response.ok) {
                ...
                await showCustomAlert('Erro', `Erro: ${result.detail}`);
                statusMsg.classList.add('hidden');
            }
        } catch (error) {
            console.error(error);
            await showCustomAlert('Erro de Conexão', 'Erro ao conectar com a API. Certifique-se de que o backend FastAPI está rodando.');
            statusMsg.classList.add('hidden');
        } finally {
...
function startAudioBackupFlow() {
    ...
    if (audios.length === 0) {
        showCustomAlert('Aviso', 'Nenhum áudio identificado no último scan.');
        return;
    }
...
async function startClassificationFlow() {
    ...
    if (!mediaFiles || mediaFiles.length === 0) {
        await showCustomAlert('Aviso', 'Nenhum arquivo de mídia identificado nesta sessão. Rode um scan primeiro.');
        return;
    }
    const imgCount = mediaFiles.filter(f => f.category === 'imagens').length;
    if (imgCount === 0) {
        await showCustomAlert('Aviso', 'Nenhuma IMAGEM identificada no último scan. A classificação por IA só roda em imagens.');
        return;
    }
    const ok = await showCustomConfirm(
    ...
    try {
        ...
        const data = await res.json();
        if (!res.ok) {
            await showCustomAlert('Erro ao Iniciar', `Erro ao iniciar: ${data.detail || 'desconhecido'}`);
            return;
        }
...
async function openTrashModalFromResults() {
    try {
        ...
        await showCustomAlert('Erro', `Erro ao buscar resultados: ${e.message}`);
    }
}
...
async function loadLatestReport(force = false) {
    ...
    try {
        ...
        if (result.status === 'success' && Array.isArray(records) && records.length > 0) {
            ...
            // Atualiza os dados de mídia a partir do relatório persistido
            const mBreakdown = result.data && result.data.media_breakdown;
            const mFiles = result.data && result.data.media_files;
            
            if (mFiles && mBreakdown) {
                mediaFiles = mFiles || [];
                ...
                if (btnTrash) btnTrash.disabled = mediaCount === 0;
                const btnAudio = document.getElementById('btn-audio-trash');
                const audioCount = (mBreakdown.audio && mBreakdown.audio.count) || 0;
                if (btnAudio) btnAudio.disabled = audioCount === 0;
                renderMediaBreakdown(mBreakdown);
                closeMediaList();
            }
        } else {
